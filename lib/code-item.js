...
export default class CodeItem {
  ...
  addMatch(tbody, match, path) {
    for (const line of match.lines) {
      ...
      if (line.kind == MATCH && line.start != null && line.end != null) {
        const start = line.start;
        const end = line.end;

        // TODO: Slice byte array instead.
        const before = document.createElement('span');
        before.textContent = `${line.bytes.slice(0, start)}`;
        const matched = document.createElement('span');
        matched.classList.add('highlight-info');
        matched.textContent = `${line.bytes.slice(start, end)}`;
        const after = document.createElement('span');
        after.textContent = `${line.bytes.slice(end)}`;
